...
(function () {
    ...
    var DICT = {
        'zh-CN': {
            ...
            'hero.description': '两个角色设定构建器 × 五张成品角色卡。全部以 Markdown 设定文件交付，即下即用，可自由修改与分享。',
            ...
            'qa.4.a': '把对应 SKILL.md 放入技能目录，提供作品名与角色名（原创角色直接给设定），跟随三阶段流程逐步确认；素材会缓存到本地 sources 目录，可跨会话续作。',
            'qa.5.q': '为什么要分批处理？',
            'qa.5.a': '两个原因：部分 agent 会限制单次最大输出上限，一次性产出全部文件容易被截断；分批落盘也让用户可以在每一批结束后及时审查与调整，再继续下一批。',
            'qa.6.q': '可以使用 web 端的 AI 产品吗？',
            'qa.6.a': '可以，但一般外部端产品会限制搜索的次数与长度。总体来说，web 端的搜索能力比不上完整 agent，长流程的检索与落盘建议使用支持技能加载的 agent 工具。',
            ...
            'declare.copyright.body': '角色设定素材的版权归各自原作版权方所有；构建器与文档由 LaoBiDeng321 个人维护，转载请注明来源。',
            'declare.disclaimer.title': '免责声明',
            'declare.disclaimer.body': 'AI 扮演产出内容不代表原作官方口径；使用生成内容产生的一切后果由使用者自行承担。',
        ...
        'en-US': {
            ...
            'hero.description': 'Two profile builders × five ready-to-run character cards. Everything ships as Markdown setting files — download, tweak and share freely.',
            ...
            'chars.indexSearch': 'Search work / character…',
            ...
            'qa.4.a': 'Put the SKILL.md into your skills directory, provide the work title and character name (or your OC draft directly), then follow the 3-stage gates. Materials are cached locally under sources/ and can resume across sessions.',
            ...
            'qa.6.a': 'Yes, but external web products usually limit the number and length of searches. Overall, their search capability falls short of a full agent — for long retrieval-and-disk workflows, an agent tool with skill loading is recommended.',
            ...
            'declare.copyright.body': 'Character materials belong to their respective rights holders. Builders and docs are maintained personally by LaoBiDeng321 — attribute the source when reuploading.',
            'declare.disclaimer.title': 'Disclaimer',
            'declare.disclaimer.body': 'AI roleplay output does not represent any official stance of the original works. Users bear full responsibility for generated content.',
